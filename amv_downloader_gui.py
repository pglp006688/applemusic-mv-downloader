...
# -*- coding: utf-8 -*-
"""
Apple Music 音乐视频下载器 GUI（中文界面，正式版）
新增：遇到目标文件存在时弹窗询问是否覆盖（是/否）。
保存为 amv_downloader_gui_zh_release_with_confirm_overwrite.py 并运行：
  pip install gamdl
  python amv_downloader_gui_zh_release_with_confirm_overwrite.py
"""
import asyncio
import threading
import queue
...
class AMVDownloaderGUI_ZH_Release:
    def __init__(self, root):
        ...
        ttk.Label(frm, text="Apple Music 链接（每行一个，可放歌/专辑/艺人链接）：").grid(column=0, row=0, sticky=W)
        self.url_text = scrolledtext.ScrolledText(frm, height=6)
        self.url_text.grid(column=0, row=1, columnspan=5, sticky="nsew", pady=4)

        ttk.Label(frm, text="Cookies 文件（Netscape 格式）：").grid(column=0, row=2, sticky=W, pady=(8,0))
        self.cookies_var = StringVar()
        self.cookies_entry = ttk.Entry(frm, textvariable=self.cookies_var, width=64)
        ...
        self.output_entry = ttk.Entry(frm, textvariable=self.output_var, width=64)
        ...
        self.ffmpeg_entry = ttk.Entry(frm, textvariable=self.ffmpeg_var, width=64)
        ...
        self.res_combo = ttk.Combobox(frm, state="readonly", width=22)
        ...
        self.stop_btn.grid(column=3, row=11, sticky=W, padx=(8,0), pady=(12,0))

        self.status_var = StringVar(value="准备就绪")
        ttk.Label(frm, textvariable=self.status_var).grid(column=0, row=12, columnspan=4, sticky=W, pady=(8,0))
        ...
        self.log_text.grid(column=0, row=14, columnspan=5, sticky="nsew", pady=(4,0))
    ...
    async def _ask_overwrite(self, path):
        ...
        # 等待用户响应（在后台线程的 asyncio loop 中等待）
        ...
        for idx, turl in enumerate(target_urls, start=1):
            ...
            try:
                async for download_item in downloader.get_download_item_from_url(turl):
                    ...
                    try:
                        # 临时根据用户选择修改 downloader.overwrite（只影响当前下载）
